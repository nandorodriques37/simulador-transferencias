...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({
    ...
    regras: {
      coberturaMaxDestinoDias: params.coberturaMaxDestinoDias ?? 0,
      coberturaMinDestinoDias: params.coberturaMinDestinoDias ?? 0,
      estrategiaDestino: params.estrategiaDestino ?? "prioridade",
      considerarPendenteOrigem: params.considerarPendenteOrigem !== false,
      arredondarCaixaFechada: params.arredondarCaixaFechada === true,
      minValorRota: params.minValorRota ?? 0,
    },
    kpis,
    rotas,
